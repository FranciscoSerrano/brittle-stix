...
function Region(regionID = "Outdonesia") {
  ...
  if (regionID == "Indonesia") {
    ...
    for (let index = 0; index < bodyPartName.length; index++) {
      document
        .querySelector(":root")
        .style.setProperty(
          `--price-${bodyPartName[index]}-str`,
          `'${pricePartInIDR[index]}'`
        );

      if (index < bgPartName.length)
        document
          .querySelector(":root")
          .style.setProperty(
            `--price-${bgPartName[index]}-str`,
            `'${priceBgInIDR[index]}'`
          );
    }
  } else {
...
function CropAnimate(bodyPartID) {
  ...
  document
    .querySelector(":root")
    .style.setProperty(`--color-${IDlowercase}`, "white"); // Move the cropping line, this function is flexible depending on the body part name
  bodyPart.classList.add("animation-change-color");
  movingText.innerHTML = `${
    bodyPartIDtoString.charAt(0) + IDlowercase.slice(1)
  }`; // Set the first letter to uppercase and change the text content bellow the cropper
...
function CropSetToDefault(dynamicCrop, dynamicCrop_up) {
  ...
  bodyPartName.forEach((element) => {
    ...
    document
      .querySelector(":root")
      .style.setProperty(`--color-${element}`, "#1793d1");
  });
...
function TotalPrice(IDlowercase, type) {
  ...
  let getPrice = getComputedStyle(
    document.querySelector(":root")
  ).getPropertyValue(`--price-${IDlowercase}-str`);
  ...
  if (type == "background") {
    ...
    isBg = true;
  } // Remove "+" from bg price string
  if (currentCurrency == "Outdonesia") {
    slicedPrice = strPrice.substring(2, strPrice.length - 1);
  } // Remove quotes and dollar sign
  else {
    slicedPrice = strPrice.substring(4, strPrice.length - 1);
  } // Remove "Rp." and quotes
...
function UpdatePrice() {
  let getSelectedPartPrice = getComputedStyle(
    document.querySelector(":root")
  ).getPropertyValue(`--price-${currentBodyPart}-str`);
  let getSelectedBgPrice = getComputedStyle(
    document.querySelector(":root")
  ).getPropertyValue(`--price-${currentBg}-str`);
  let { slicedPartPrice, slicedBgPrice } = "";
  let totalPrice = 0;

  if (currentCurrency == "Outdonesia") {
    slicedPartPrice = getSelectedPartPrice.substring(
      2,
      getSelectedPartPrice.length - 1
    );
    slicedBgPrice = getSelectedBgPrice.substring(
      3,
      getSelectedBgPrice.length - 1
    );
  } else {
    slicedPartPrice = getSelectedPartPrice.substring(
      4,
  ...
  document.querySelector(":root").style.setProperty(
    ...
    (function () {
      if (currentCurrency == "Outdonesia") {
        ...
        return `'Rp.${parseInt(totalPrice)}.000'`;
      }
    })()
  );
}
...
function BgAnimate(bgID) {
  ...
  document
    .querySelector(":root")
    .style.setProperty(`--color-${IDlowercase}`, "white"); // Set selected price color to white
...
function BgSetToDefault() {
  ...
  bgPartName.forEach((backgrounds) => {
    ...
    document.querySelector(
      `.img-${backgrounds}`
    ).className = `img-${backgrounds}`;
    document
      .querySelector(":root")
      .style.setProperty(`--color-${backgrounds}`, "#FF4C01");
  });
}
...
function Commercial() {
  ...
  if (commercial != 1) {
    commercial = 1;
    commercalButton.className = "commercial-check-1 box-bold";
  } else {
    commercial = 1.5;
    commercalButton.classList.add("animation-commercial");
  }
...
function ForwardToForm() {
  ...
  let fCommercial = (function () {
    if (commercial == 1) {
      return "No, I only use it for non-commercial purposes.";
    } else {
      return "Yes, I want it to be used as commercial purposes.";
    }
  })();
  ...
  let totalPrice_raw = window
    .getComputedStyle(document.querySelector(":root"))
    .getPropertyValue("--total-price-str");
  ...
  window.open(
    `https://docs.google.com/forms/d/e/1FAIpQLSePoEfMdtBQeu77gCH41TZ6KMfOeQ2uUVPbD1SLZBG9afAUmw/viewform?${Parts}=${fParts[iParts]}&${Bg}=${fBg[iBg]}&${Canvas}=${fCanvas[iCanvas]}&${Commercial}=${fCommercial}&${Currency}=${fCurrency[iCurrency]}&${Price}=${updatedTotalPrice}`,
...
if (window.attachEvent) {
  window.attachEvent("onresize", function () {
    if (window.innerWidth < 800)
      document.querySelector(":root").style.setProperty("--x-offset", "25vw");
    else document.querySelector(":root").style.setProperty("--x-offset", "8vw");
  });
...
window.addEventListener("scroll", function () {
  if (onceBg) {
    onceBg = false;
    bg = document.getElementById("header-img");
  }
